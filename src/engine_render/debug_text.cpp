...
std::vector<std::filesystem::path> candidate_font_paths(const char *font_filename) {
    namespace fs = std::filesystem;
...
bool load_file_bytes(const std::filesystem::path &path, std::vector<uint8_t> &out_bytes) {
...
bool load_raster_font(RasterFont &font) {
    ...
    for (const std::filesystem::path &path : candidate_font_paths("IBMPlexMono-Regular.ttf")) {
    ...
    const float scale = stbtt_ScaleForPixelHeight(&info, static_cast<float>(font.pixel_height));
    int ascent = 0;
    int descent = 0;
    int line_gap = 0;
    ...
    for (int code = 32; code < 127; ++code) {
        ...
        int xoff = 0;
        int yoff = 0;
        unsigned char *bitmap = stbtt_GetCodepointBitmap(&info, 0.0f, scale, code, &glyph.width, &glyph.height, &xoff, &yoff);
        glyph.offset_x = xoff;
        glyph.offset_y = yoff;
        if (bitmap && glyph.width > 0 && glyph.height > 0) {
            glyph.bitmap.assign(bitmap, bitmap + static_cast<size_t>(glyph.width * glyph.height));
        }
...
void add_screen_pixel_rect_ndc(
    RenderMesh &mesh,
    float origin_x_ndc,
    float origin_y_ndc,
    float x,
...
void append_raster_glyph_world(
    RenderMesh &mesh,
    ...
    const float glyph_y = pen_y + static_cast<float>(font.ascent + glyph.offset_y);
    for (int row = 0; row < glyph.height; ++row) {
        int run_start = -1;
        for (int col = 0; col <= glyph.width; ++col) {
            const bool on =
                col < glyph.width &&
                glyph.bitmap[static_cast<size_t>(row * glyph.width + col)] >= k_font_alpha_threshold;
            if (on && run_start < 0) {
                run_start = col;
                ...
                add_pixel_rect(
                    ...
                    static_cast<float>(col - run_start),
                    1.0f,
                    cell_size,
                    color);
                run_start = -1;
...
void append_raster_glyph_screen(
    RenderMesh &mesh,
    const RasterFont &font,
    char c,
    float origin_x_ndc,
...
RenderMesh build_camera_text_mesh(const Camera &camera, const std::string &text) {
    ...
    if (font) {
        ...
        const float font_cell_size = target_height / static_cast<float>(std::max(1, font->pixel_height));
        ...
        for (char c : text) {
            ...
            append_raster_glyph_world(mesh, *font, c, anchor, right, up, pen_x, pen_y, font_cell_size, color);
    ...
    for (char c : text) {
        ...
        for (int row = 0; row < 7; ++row) {
            for (int col = 0; col < 5; ++col) {
                ...
                add_pixel_rect(mesh, anchor, right, up, pen_x + static_cast<float>(col), pen_y + static_cast<float>(row), 1.0f, 1.0f, cell_size, color);
            }
        }
...
RenderMesh build_screen_text_mesh(
    ...
    if (font) {
        ...
        const float font_cell_size_ndc = target_height_ndc / static_cast<float>(std::max(1, font->pixel_height));
        for (char c : text) {
            if (c == '\n') {
                pen_x = 0.0f;
                pen_y += static_cast<float>(font->line_height) * std::max(1.0f, line_spacing_scale);
                continue;
            }
            append_raster_glyph_screen(
                mesh,
                *font,
                c,
                origin_x_ndc,
