...
private:
  ...
  void update_dynamic_buffer_set(const std::vector<RenderVertex> &vertices,
                                 const std::vector<uint32_t> &indices,
                                 VkBuffer &inout_vertex_buffer,
                                 VkDeviceMemory &inout_vertex_memory,
                                 VkDeviceSize &inout_vertex_capacity,
                                 VkBuffer &inout_index_buffer,
                                 VkDeviceMemory &inout_index_memory,
                                 VkDeviceSize &inout_index_capacity);
  void create_mesh_buffers(const std::vector<RenderVertex> &vertices,
                           const std::vector<uint32_t> &indices,
                           VkBuffer &out_vertex_buffer,
                           VkDeviceMemory &out_vertex_memory,
                           VkBuffer &out_index_buffer,
                           VkDeviceMemory &out_index_memory);
  void create_empty_mesh_buffers(VkDeviceSize vertex_size,
                                 VkDeviceSize index_size,
                                 VkBuffer &out_vertex_buffer,
                                 VkDeviceMemory &out_vertex_memory,
                                 VkBuffer &out_index_buffer,
                                 VkDeviceMemory &out_index_memory);
  void upload_mesh_buffers(const std::vector<RenderVertex> &vertices,
                           const std::vector<uint32_t> &indices,
                           VkDeviceMemory vertex_memory,
                           VkDeviceMemory index_memory) const;
  void destroy_mesh_buffers(VkBuffer &inout_vertex_buffer,
                            VkDeviceMemory &inout_vertex_memory,
                            VkBuffer &inout_index_buffer,
                            VkDeviceMemory &inout_index_memory);
  void destroy_scene_buffers();
  void wait_for_all_frames_in_flight();
  ...
  VkDeviceSize debug_world_vertex_capacity = 0;
  VkDeviceSize debug_world_index_capacity = 0;
  ...
  VkDeviceSize debug_screen_vertex_capacity = 0;
  VkDeviceSize debug_screen_index_capacity = 0;
  uint32_t static_index_count = 0;
  uint32_t debug_world_index_count = 0;
  uint32_t debug_screen_index_count = 0;
  std::array<glm::mat4, 2> current_view_proj = {glm::mat4(1.0f),
                                                glm::mat4(1.0f)};
  std::array<glm::vec4, 2> current_viewports = {glm::vec4(0.0f),
                                                glm::vec4(0.0f)};
