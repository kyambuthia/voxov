...
std::vector<char>
read_file_with_fallback(const std::vector<std::string> &candidates) {
...
void VulkanRenderer::upload_scene(const RenderScene &scene) {
  ...
  auto append_mesh = [&](const RenderMesh &mesh) {
    ...
    static_vertices.insert(static_vertices.end(), mesh.vertices.begin(),
                           mesh.vertices.end());
...
void VulkanRenderer::update_dynamic_meshes(const RenderMesh &debug_world,
  ...
  update_dynamic_buffer_set(
      debug_world_vertices, debug_world_indices, debug_world_vertex_buffer,
      debug_world_vertex_memory, debug_world_vertex_capacity,
      debug_world_index_buffer, debug_world_index_memory,
      debug_world_index_capacity);
  update_dynamic_buffer_set(
      debug_screen_vertices, debug_screen_indices, debug_screen_vertex_buffer,
      debug_screen_vertex_memory, debug_screen_vertex_capacity,
      debug_screen_index_buffer, debug_screen_index_memory,
      debug_screen_index_capacity);
}
...
void VulkanRenderer::end_frame() {
  ...
  if (vkQueueSubmit(graphics_queue, 1, &submit, in_flight[frame_index]) !=
      VK_SUCCESS) {
...
void VulkanRenderer::create_instance() {
  ...
  if (!exts || ext_count == 0) {
    throw std::runtime_error(
        "glfwGetRequiredInstanceExtensions returned no extensions");
  }
  ...
  if (enable_validation_layers) {
    ...
    if (!found) {
      std::fprintf(stderr,
                   "Validation layer not found; continuing without it\n");
...
void VulkanRenderer::setup_debug_messenger() {
  ...
  if (create_fn &&
      create_fn(instance, &info, nullptr, &debug_messenger) != VK_SUCCESS) {
...
void VulkanRenderer::create_swapchain() {
  ...
  for (uint32_t i = 0; i < image_count; ++i) {
    ...
    if (vkCreateImageView(device, &view, nullptr, &swapchain_image_views[i]) !=
        VK_SUCCESS) {
...
void VulkanRenderer::create_depth_resources() {
  ...
  alloc.memoryTypeIndex =
      find_memory_type(mem.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
  ...
  if (vkCreateImageView(device, &view, nullptr, &depth_image_view) !=
      VK_SUCCESS) {
...
void VulkanRenderer::create_framebuffers() {
  ...
  for (size_t i = 0; i < swapchain_image_views.size(); ++i) {
    std::array<VkImageView, 2> attachments = {swapchain_image_views[i],
                                              depth_image_view};
...
void VulkanRenderer::create_pipeline() {
  const auto vert = read_file_with_fallback(
      {"triangle.vert.spv", "../triangle.vert.spv", "build/triangle.vert.spv",
       "./build/triangle.vert.spv"});
  const auto frag = read_file_with_fallback(
      {"triangle.frag.spv", "../triangle.frag.spv", "build/triangle.frag.spv",
  ...
  if (vkCreatePipelineLayout(device, &layout, nullptr, &pipeline_layout) !=
      VK_SUCCESS) {
  ...
  if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &info, nullptr,
                                &pipeline_no_depth_no_cull) != VK_SUCCESS) {
    throw std::runtime_error(
        "failed to create debug no-depth no-cull pipeline");
  }
...
void VulkanRenderer::create_command_pool() {
  ...
  if (vkCreateCommandPool(device, &info, nullptr, &command_pool) !=
      VK_SUCCESS) {
...
void VulkanRenderer::create_command_buffers() {
  ...
  if (vkAllocateCommandBuffers(device, &alloc, command_buffers.data()) !=
      VK_SUCCESS) {
...
void VulkanRenderer::recreate_swapchain() {
  ...
  if (render_finished.size() != swapchain_images.size()) {
    ...
    for (size_t i = 0; i < render_finished.size(); ++i) {
      if (vkCreateSemaphore(device, &sem, nullptr, &render_finished[i]) !=
          VK_SUCCESS) {
        throw std::runtime_error(
...
void VulkanRenderer::create_sync_objects() {
  ...
  for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
    if (vkCreateSemaphore(device, &sem, nullptr, &image_available[i]) !=
            VK_SUCCESS ||
...
void VulkanRenderer::destroy_scene_buffers() {
  destroy_mesh_buffers(static_vertex_buffer, static_vertex_memory,
                       static_index_buffer, static_index_memory);
  destroy_mesh_buffers(debug_world_vertex_buffer, debug_world_vertex_memory,
                       debug_world_index_buffer, debug_world_index_memory);
  debug_world_vertex_capacity = 0;
  debug_world_index_capacity = 0;
  destroy_mesh_buffers(debug_screen_vertex_buffer, debug_screen_vertex_memory,
                       debug_screen_index_buffer, debug_screen_index_memory);
  debug_screen_vertex_capacity = 0;
  debug_screen_index_capacity = 0;
}

void VulkanRenderer::create_mesh_buffers(
    const std::vector<RenderVertex> &vertices,
    const std::vector<uint32_t> &indices, VkBuffer &out_vertex_buffer,
    VkDeviceMemory &out_vertex_memory, VkBuffer &out_index_buffer,
    VkDeviceMemory &out_index_memory) {
  VkDeviceSize vb_size =
      static_cast<VkDeviceSize>(vertices.size() * sizeof(RenderVertex));
  ...
  vb_alloc.memoryTypeIndex = find_memory_type(
      vb_mem.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                 VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
  if (vkAllocateMemory(device, &vb_alloc, nullptr, &out_vertex_memory) !=
      VK_SUCCESS) {
  ...
  ib_alloc.memoryTypeIndex = find_memory_type(
      ib_mem.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                 VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
  if (vkAllocateMemory(device, &ib_alloc, nullptr, &out_index_memory) !=
      VK_SUCCESS) {
...
void VulkanRenderer::create_empty_mesh_buffers(
    VkDeviceSize vertex_size, VkDeviceSize index_size,
    VkBuffer &out_vertex_buffer, VkDeviceMemory &out_vertex_memory,
    VkBuffer &out_index_buffer, VkDeviceMemory &out_index_memory) {
  VkBufferCreateInfo vb{};
  vb.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
  vb.size = vertex_size;
  vb.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
...
void VulkanRenderer::upload_mesh_buffers(
    const std::vector<RenderVertex> &vertices,
    const std::vector<uint32_t> &indices, VkDeviceMemory vertex_memory,
    VkDeviceMemory index_memory) const {
  const VkDeviceSize vb_size =
      static_cast<VkDeviceSize>(vertices.size() * sizeof(RenderVertex));
  if (vb_size > 0) {
    void *vb_ptr = nullptr;
    vkMapMemory(device, vertex_memory, 0, vb_size, 0, &vb_ptr);
    std::memcpy(vb_ptr, vertices.data(), static_cast<size_t>(vb_size));
  ...
  if (ib_size > 0) {
    void *ib_ptr = nullptr;
    vkMapMemory(device, index_memory, 0, ib_size, 0, &ib_ptr);
    std::memcpy(ib_ptr, indices.data(), static_cast<size_t>(ib_size));
    vkUnmapMemory(device, index_memory);
  }
}

void VulkanRenderer::destroy_mesh_buffers(VkBuffer &inout_vertex_buffer,
...
void VulkanRenderer::wait_for_all_frames_in_flight() {
  if (device == VK_NULL_HANDLE || in_flight.empty()) {
    return;
  }
  vkWaitForFences(device, static_cast<uint32_t>(in_flight.size()),
                  in_flight.data(), VK_TRUE, UINT64_MAX);
}

void VulkanRenderer::update_dynamic_buffer_set(
    const std::vector<RenderVertex> &vertices,
    const std::vector<uint32_t> &indices, VkBuffer &inout_vertex_buffer,
    VkDeviceMemory &inout_vertex_memory, VkDeviceSize &inout_vertex_capacity,
    VkBuffer &inout_index_buffer, VkDeviceMemory &inout_index_memory,
    VkDeviceSize &inout_index_capacity) {
  if (device == VK_NULL_HANDLE) {
    return;
  }
  ...
  if (vertices.empty() || indices.empty()) {
    if (inout_vertex_buffer != VK_NULL_HANDLE ||
        inout_index_buffer != VK_NULL_HANDLE) {
      wait_for_all_frames_in_flight();
      destroy_mesh_buffers(inout_vertex_buffer, inout_vertex_memory,
                           inout_index_buffer, inout_index_memory);
    }
    inout_vertex_capacity = 0;
    inout_index_capacity = 0;
    return;
  }

  const VkDeviceSize needed_vertex_bytes =
      static_cast<VkDeviceSize>(vertices.size() * sizeof(RenderVertex));
  const VkDeviceSize needed_index_bytes =
      static_cast<VkDeviceSize>(indices.size() * sizeof(uint32_t));
  const bool needs_realloc = inout_vertex_buffer == VK_NULL_HANDLE ||
                             inout_index_buffer == VK_NULL_HANDLE ||
                             needed_vertex_bytes > inout_vertex_capacity ||
                             needed_index_bytes > inout_index_capacity;
  ...
  if (needs_realloc) {
    ...
    const VkDeviceSize next_vertex_capacity =
        std::max(needed_vertex_bytes, inout_vertex_capacity > 0
                                          ? inout_vertex_capacity * 2
                                          : needed_vertex_bytes);
    const VkDeviceSize next_index_capacity = std::max(
        needed_index_bytes, inout_index_capacity > 0 ? inout_index_capacity * 2
                                                     : needed_index_bytes);
    create_empty_mesh_buffers(next_vertex_capacity, next_index_capacity,
                              inout_vertex_buffer, inout_vertex_memory,
                              inout_index_buffer, inout_index_memory);
    inout_vertex_capacity = next_vertex_capacity;
    inout_index_capacity = next_index_capacity;
  }

  upload_mesh_buffers(vertices, indices, inout_vertex_memory,
                      inout_index_memory);
}
...
void VulkanRenderer::create_scene_buffers() {
  ...
  if (!static_vertices.empty() && !static_indices.empty()) {
    create_mesh_buffers(static_vertices, static_indices, static_vertex_buffer,
                        static_vertex_memory, static_index_buffer,
                        static_index_memory);
  }

  if (!debug_world_vertices.empty() && !debug_world_indices.empty()) {
    create_mesh_buffers(debug_world_vertices, debug_world_indices,
                        debug_world_vertex_buffer, debug_world_vertex_memory,
                        debug_world_index_buffer, debug_world_index_memory);
    debug_world_vertex_capacity = static_cast<VkDeviceSize>(
        debug_world_vertices.size() * sizeof(RenderVertex));
    debug_world_index_capacity = static_cast<VkDeviceSize>(
        debug_world_indices.size() * sizeof(uint32_t));
  }

  if (!debug_screen_vertices.empty() && !debug_screen_indices.empty()) {
    create_mesh_buffers(debug_screen_vertices, debug_screen_indices,
                        debug_screen_vertex_buffer, debug_screen_vertex_memory,
                        debug_screen_index_buffer, debug_screen_index_memory);
    debug_screen_vertex_capacity = static_cast<VkDeviceSize>(
        debug_screen_vertices.size() * sizeof(RenderVertex));
    debug_screen_index_capacity = static_cast<VkDeviceSize>(
        debug_screen_indices.size() * sizeof(uint32_t));
  }
}

void VulkanRenderer::record_command_buffer(VkCommandBuffer cmd,
  ...
  for (uint32_t i = 0; i < current_view_count; ++i) {
    ...
    const int vx =
        static_cast<int>(vp.x * static_cast<float>(swapchain_extent.width));
    const int vy =
        static_cast<int>(vp.y * static_cast<float>(swapchain_extent.height));
    const uint32_t vw =
        std::max(1u, static_cast<uint32_t>(
                         vp.z * static_cast<float>(swapchain_extent.width)));
    const uint32_t vh =
        std::max(1u, static_cast<uint32_t>(
                         vp.w * static_cast<float>(swapchain_extent.height)));
    ...
    if (static_vertex_buffer != VK_NULL_HANDLE &&
        static_index_buffer != VK_NULL_HANDLE && static_index_count > 0) {
    ...
    if (debug_world_vertex_buffer != VK_NULL_HANDLE &&
        debug_world_index_buffer != VK_NULL_HANDLE &&
        debug_world_index_count > 0) {
      vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                        current_debug_xray ? pipeline_no_depth : pipeline);
      const VkDeviceSize offsets[] = {0};
      vkCmdBindVertexBuffers(cmd, 0, 1, &debug_world_vertex_buffer, offsets);
      vkCmdBindIndexBuffer(cmd, debug_world_index_buffer, 0,
                           VK_INDEX_TYPE_UINT32);
      vkCmdDrawIndexed(cmd, debug_world_index_count, 1, 0, 0, 0);
    }

    if (debug_screen_vertex_buffer != VK_NULL_HANDLE &&
        debug_screen_index_buffer != VK_NULL_HANDLE &&
        debug_screen_index_count > 0) {
      vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                        pipeline_no_depth_no_cull);
      PushConstants screen_push{};
      // Match the desktop screen-space text orientation across renderers.
      screen_push.view_proj =
          glm::scale(glm::mat4(1.0f), glm::vec3(1.0f, -1.0f, 1.0f));
      vkCmdPushConstants(cmd, pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0,
                         sizeof(PushConstants), &screen_push);
      const VkDeviceSize offsets[] = {0};
      vkCmdBindVertexBuffers(cmd, 0, 1, &debug_screen_vertex_buffer, offsets);
      vkCmdBindIndexBuffer(cmd, debug_screen_index_buffer, 0,
                           VK_INDEX_TYPE_UINT32);
