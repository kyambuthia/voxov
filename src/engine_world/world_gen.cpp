...
WorldGenerator::WorldGenerator(uint64_t world_seed)
    : seed(world_seed) {}

uint64_t WorldGenerator::world_seed() const {
    return seed;
}

uint64_t WorldGenerator::chunk_seed(VoxelChunkCoord2D coord) const {
    uint64_t mixed = seed;
    mixed ^= static_cast<uint64_t>(static_cast<uint32_t>(coord.x)) * 0x9e3779b97f4a7c15ull;
    mixed ^= static_cast<uint64_t>(static_cast<uint32_t>(coord.z)) * 0xc2b2ae3d27d4eb4full;
...
float WorldGenerator::sample_height(float world_x, float world_z) const {
    ...
    const float seed_phase_z = static_cast<float>(((seed >> 16u) & 0xffffu)) * 0.00017f;
