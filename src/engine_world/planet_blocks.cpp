...
void BlockWorld::build_shells() {
    ...
    // Derive shell thickness from the configured generator bounds. Compact
    // planets must not inherit tens of metres of large-world relief.
    const double max_terrain =
        static_cast<double>(config_.terrain_max_height +
                            config_.terrain_shell_margin) *
        config_.block_size;
...
void BlockWorld::init(const BlockWorldConfig &cfg) {
    ...
    config_.terrain_base_height = std::max(0.0f, config_.terrain_base_height);
    config_.terrain_amplitude = std::max(0.0f, config_.terrain_amplitude);
    config_.terrain_min_height = std::max(0, config_.terrain_min_height);
    config_.terrain_max_height =
        std::max(config_.terrain_min_height, config_.terrain_max_height);
    config_.terrain_shell_margin = std::max(1, config_.terrain_shell_margin);
...
int32_t BlockWorld::terrain_height_at(const glm::dvec3 &world_dir) const {
    ...
    return noise.terrain_height(
        world_dir, config_.terrain_base_height, config_.terrain_amplitude,
        frequency, config_.terrain_min_height, config_.terrain_max_height);
}
...
int32_t SphereNoise3D::terrain_height(const glm::dvec3 &direction,
    ...
    const float h = terrain_height_raw(direction, base_height, amplitude,
                                       base_frequency);
    const int32_t lower = std::max(0, min_height);
    const int32_t upper = std::max(lower, max_height);
    return std::clamp(static_cast<int32_t>(std::floor(h)), lower, upper);
}
...
double planet_flight_clipmap_half_extent(double planet_radius,
    ...
    const double radius = std::max(1.0, planet_radius);
    const double min_extent = std::min(768.0, radius * 0.25);
    const double altitude_bias = std::min(512.0, radius * 0.25);
    const double requested = std::min(
        radius * 0.08, std::max(0.0, camera_altitude) * 1.5 + altitude_bias);
    return std::clamp(requested, min_extent,
...
RenderMesh build_planet_flight_clipmap(
    ...
    const double base_half_extent =
        planet_flight_clipmap_half_extent(radius, camera_altitude);
