...
void JoltSolver::init(const EnginePhysicsSettings &settings) {
...
void JoltSolver::step(float dt_seconds) {
    auto *system = static_cast<PhysicsSystem *>(physics_system);
    if (!system || !temp_allocator || !job_system) {
        return;
    }
